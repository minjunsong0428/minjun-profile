# 송민준 Profile

안녕하세요. 코딩, 디자인, AI, IT, 보안에 관심이 많은 송민준입니다.

이 저장소는 저를 소개하고, 제가 사용하는 여러 연락 채널을 한곳에 모아두기 위해 만든 개인 프로필 사이트입니다. 카카오톡, 디스코드, 인스타그램, GitHub 링크를 통해 더 편하게 연결될 수 있도록 구성했습니다.

## About Me

저는 작은 아이디어를 실제로 써볼 수 있는 결과물로 만드는 과정을 좋아합니다. 웹 화면을 만들고, 기능을 붙이고, 사람들이 편하게 사용할 수 있는 형태로 다듬는 일에 흥미가 있습니다.

특히 코딩과 디자인을 함께 생각하는 것을 좋아합니다. 단순히 작동하는 것에서 끝나는 것이 아니라, 보기 쉽고 누르기 편하며 사용하는 사람이 자연스럽게 이해할 수 있는 결과물을 만들고 싶습니다.

## Interests

- 코딩: 아이디어를 실제 기능과 웹사이트로 구현하는 것에 관심이 있습니다.
- 디자인: 보기 좋고 사용하기 편한 화면, 3D 모델링, Adobe 프로그램을 활용한 편집 과정에 관심이 있습니다.
- AI: 공부, 제작, 자동화에 AI를 활용하는 방법을 탐구하고 있습니다.
- 창작: 작은 생각을 콘텐츠나 프로젝트로 발전시키는 것을 좋아합니다.
- IT: 새로운 기술과 디지털 서비스가 생활을 바꾸는 방식에 관심이 있습니다.
- 보안: 윤리적 해킹과 웹 보안을 배우며 시스템을 더 안전하게 만드는 방법에 관심이 있습니다.

## Tools

- HTML / CSS / JS: 웹 화면과 간단한 기능을 직접 구현하며 배우고 있습니다.
- GitHub: 코드 기록, 저장소 관리, 프로젝트 정리에 활용하고 있습니다.
- Adobe: 편집과 디자인 표현을 익히는 데 관심이 있습니다.
- Blender / 3D: 모델링과 공간, 형태를 만드는 과정을 배우고 있습니다.
- AI 도구: 공부, 아이디어 정리, 제작 효율을 높이는 데 활용하고 있습니다.
- 보안 기초: 웹 보안과 윤리적 해킹의 기본 개념을 배우고 있습니다.

## Contact

문의나 소통은 디스코드, 인스타그램, 카카오톡을 가장 편하게 사용합니다.

- Discord: `minjun_song`, `iam_not_minjun_song`, `minjun._.song`
- Instagram: `minjun._.song`
- KakaoTalk: `open.kakao.com/o/s9gKj73h`
- GitHub: `minjunsong0428`, `songminjun0428`
- Email: `minjunsong0428@gmail.com`

Thanks for visiting.
